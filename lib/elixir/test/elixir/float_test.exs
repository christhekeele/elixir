...
defmodule FloatTest do
  ...
  # Implementation of test suite provided here:
  #   https://floating-point-gui.de/errors/NearlyEqualsTest.java
  describe "is_close/2" do
    test "with identical inputs" do
      assert Float.is_close(0.0, 0.0)
      assert Float.is_close(0.1, 0.1)
      assert Float.is_close(1_000_000.0, 1_000_000.0)
      assert Float.is_close(-1_000_000.0, -1_000_000.0)
    end
    ...
    test "with inputs close to 0" do
      assert Float.is_close(0.3, 0.30000003)
      assert Float.is_close(-0.3, -0.30000003)
    end
