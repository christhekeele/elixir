...
defmodule ExUnit.OnExitHandler do
  ...
  @spec run(pid, %ExUnit.Test.Result{} | %ExUnit.TestModule.Result{}, timeout) ::
          :ok | {Exception.kind(), term, Exception.stacktrace()}
  def run(pid, result, timeout) when is_pid(pid) do
    [{^pid, sup, callbacks}] = :ets.take(@name, pid)
    error = terminate_supervisor(sup, timeout)
    exec_on_exit_callbacks(Enum.reverse(callbacks), result, timeout, error)
  end
  ...
  defp exec_on_exit_callbacks(callbacks, result, timeout, error) do
    {runner_pid, runner_monitor, error} =
      Enum.reduce(callbacks, {nil, nil, error}, &exec_on_exit_callback(&1, result, timeout, &2))
  ...
  def on_exit_runner_loop do
    receive do
      {:run, from, callback, result} ->
        send(from, {self(), exec_callback(callback, result)})
        on_exit_runner_loop()
    end
  end
  ...
  defp exec_callback(callback, result) do
    callback.(result)
    nil
  catch
    kind, error ->
